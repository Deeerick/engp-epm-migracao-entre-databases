...
SELECT
    ...
    CASE 
        ...
        WHEN CHARINDEX('NEXE', STATUS_SISTEMA_ORDEM + ' ' + STATUS_USUARIO_ORDEM) > 0 OR 
                ...
                CHARINDEX('ELIM', STATUS_SISTEMA_ORDEM + ' ' + STATUS_USUARIO_ORDEM) > 0 THEN '6 - Ordem Eliminada/ Não Executada'
    ...
    END AS SIT_ORDEM,
    CASE
        WHEN CHARINDEX('ABER', STATUS_SISTEMA_ORDEM + ' ' + STATUS_USUARIO_ORDEM) > 0
        AND ORDEM IS NOT NULL THEN 1
        ELSE 0
    END AS ABER,
    CASE
        WHEN CHARINDEX('LIB', STATUS_SISTEMA_ORDEM + ' ' + STATUS_USUARIO_ORDEM) > 0
        AND CHARINDEX('CNPA', STATUS_SISTEMA_ORDEM + ' ' + STATUS_USUARIO_ORDEM) = 0
        AND CHARINDEX('CONF', STATUS_SISTEMA_ORDEM + ' ' + STATUS_USUARIO_ORDEM) = 0
        AND ORDEM IS NOT NULL THEN 1
        ELSE 0
    END AS LIB,
    CASE
    ...
    END AS LIB_CONF,
    CASE
        WHEN CHARINDEX('ENTE', STATUS_SISTEMA_ORDEM + ' ' + STATUS_USUARIO_ORDEM) > 0
        AND ORDEM IS NOT NULL THEN 1
        ELSE 0
    END AS ENTE,
    CASE
        WHEN CHARINDEX('ENCE', STATUS_SISTEMA_ORDEM + ' ' + STATUS_USUARIO_ORDEM) > 0
        AND ORDEM IS NOT NULL THEN 1
        ELSE 0
    END AS ENCE,
    DATA_BASE_FIM,
